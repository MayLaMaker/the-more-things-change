...
@export var enable_spine_rotation := true
func _process(delta):
	...
	if hand_right:hand_right.translate(right_delta)
	...
	if enable_spine_rotation:
		...
		if spine_id != -1:
			...
			var body_pos := Body_Origin.global_transform.origin# Hand direction (XZ)
			var hand_dir := right_pos - left_pos
			hand_dir.y = 0.0
			if hand_dir.length() < 0.001:return
			...
			skeleton.set_bone_global_pose_override(spine_id,spine_pose,1.0,true)
