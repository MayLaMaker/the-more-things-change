...
@export var use_servo_position_controller: bool = true
var physics_bones = []
...
func _physics_process(delta):current_delta = delta
func hookes_law(displacement: Vector3, current_velocity: Vector3, stiffness: float, damping: float) -> Vector3:
	return (stiffness * displacement) - (damping * current_velocity)
func servo_pd(displacement: Vector3, current_velocity: Vector3, position_gain: float, velocity_gain: float) -> Vector3:
	var desired_velocity = displacement * position_gain
	return (desired_velocity - current_velocity) * velocity_gain
func _on_skeleton_3d_skeleton_updated() -> void:
	for b: PhysicalBone3D in physics_bones:
		...
		var force: Vector3
		if use_servo_position_controller:force = servo_pd(position_difference, b.linear_velocity, linear_spring_stiffness, linear_spring_damping)
		else:force = hookes_law(position_difference, b.linear_velocity, linear_spring_stiffness, linear_spring_damping)
